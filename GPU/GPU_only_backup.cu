...
int main(int argc, char **argv)
{
	...
	if (error!= cudaSuccess)
				printf("erro_6: %s\n", cudaGetErrorString(error));
	//cudaMemcpy(d_output, h_output, (h_offset[filelength]/8)*sizeof(unsigned char), cudaMemcpyHostToDevice);
	...
	if (error!= cudaSuccess)
				printf("erro_8: %s\n", cudaGetErrorString(error));
	//cudaMemcpyToSymbol(d_table, h_table, 256 * sizeof(table_struct), cudaMemcpyHostToDevice);
